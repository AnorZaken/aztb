...
public class StringLoader<T extends JavaPlugin>
{
	 ...
	 * <p/>This enum is used by {@link StringLoader#loadStrings(CommandSender, MissingAction, boolean)}.
	 * @author AnorZaken
	 * @see StringLoader#loadStrings(CommandSender, MissingAction, boolean)
	 */
	...
	private IStringProvider msgProvider; //<-- neverNull
	private final ArrayList<ISubscriber> subscribers = new ArrayList<ISubscriber>();
	private final YAMLLoader<T> loader; //<-- neverNull
	private String yamlHeader = null;
	private boolean reportUnrecognized = false;
	 ...
	 */
	public StringLoader(final T plugin, final String filename, final IStringProvider stringProvider)
	{
	 ...
	 * in the yaml).
	 * @param failIfFileNotFound if this is {@code true} {@link #loadStrings(CommandSender, MissingAction, boolean)} 
	 * will treat FileNotFound as an error and report it to the {@code sender} accordingly. (The {@code sender} will 
	 * be notified regardless, but it wont look like an error occurred -- rather a simple notification.)
	 * @return {@code true} if yaml-loading succeeded or FileNotFound occurred and {@code failIfFileNotFound == false},
	 * otherwise {@code false}.
	 */
	public boolean loadStrings(final CommandSender sender, MissingAction missingAction, final boolean failIfFileNotFound)
	{
		if (sender != null) {
			String s = msgProvider.cfg_loading(loader.filename);
			if (s != null)
		...
		YAMLLoader.YAMLResult yamlResult = loader.loadYaml(sender, failIfFileNotFound, msgProvider.cfg_errorLoading());
		...
		if (!yamlResult.isFileFound) {
			if (sender != null) {
				final String s = msgProvider.cfg_fileLoadFail(loader.filename);
				sender.sendMessage(s);
			}
			...
			if (sender != null) {
				final String s = msgProvider.cfg_fileYAMLInvalid(loader.filename);
				sender.sendMessage(s);
			}
			return false;
		}
				...
				for (IStringToLoad istl : istlArr)
				{
					...
					if (yaml.contains(cfg))
					{
						...
						else if (sender != null)
						{
							...
							final String s = msgProvider.cfg_warnMalformed(cfg, loader.filename);
							sender.sendMessage(s);
						}
					}
					else
					{
						++missingCount;
						if (sender != null) {
							String s = msgProvider.cfg_entryMissing(cfg, loader.filename);
							if (s != null)
								sender.sendMessage(s);
						}
		...
		if (reportUnrecognized && sender != null)
		{
			...
			if (unrecognizedCount > 0) {
				String s = msgProvider.cfg_unrecognizedKeys(unrecognizedCount, loader.filename);
				if (s != null)
					sender.sendMessage(s);
			}
		}
	 ...
	 * @param sender {@link CommandSender} to send messages to, or {@code null} if silent operation is desired.
	 * @param allowOverwrite if this is {@code false} and error-message will be displayed if the file already exist.
	 * @param templateFilename the name of the file to save to
	 * @return {@code true} if saving succeeded, otherwise {@code false}.
	 */
	public boolean saveStrings(final CommandSender sender, final boolean allowOverwrite, final String templateFilename)
	{
		if (loader.plugin.getDataFolder() == null) //Maybe this should throw?
			return false; //Triggering this would probably indicate plugin being accessed at inappropriate server state
		
		if (templateFilename == null || templateFilename.length() == 0)
			throw new IllegalArgumentException("templateFilename can not be null or empty!");
